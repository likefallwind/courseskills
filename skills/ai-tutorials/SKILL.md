...
注意：知识点的名称要清晰准确，最好单独能够表示对应知识，不能简单以项目名称、任务描述等作为知识点。以下不算知识点：“完成一个完整项目”、“声音”。
...
- 每节课一个文件夹：`lesson{i}/`
- 每节课产出三个核心文件：
  - `outline.md`：本节课的**概要**，简要介绍主要知识点，不做详细展开
  - `handout.md`：本节课的**详细讲义**，对知识点进行充分展开和讲解
  - `project.md`：本节课的项目内容描述
- 注意，项目可能是多个文件，其中`project.md`主要是项目内容描述。关于项目如果涉及到代码，可以在该课程下新建一个`src/`目录保存，该目录存放项目代码的脚手架，可以给出提示，但是不要直接给答案。如果涉及到答案，可以新建一个`answer/`目录保存。如果涉及到其他也可以新建目录并保存文件。不鼓励什么都塞到`project.md`中。
...
```
lesson1/
├── outline.md     ← 本节概要，简要介绍知识点
├── handout.md     ← 本节详细讲义
├── project.md
|-─ src/            ← 项目代码脚手架，给出提示但不直接给答案
...
**可读性与质量检查**
- 概要 (outline)：是否准确覆盖本节核心知识点？是否真的"简要"，未陷入细节？
- 详细讲义 (handout)：逻辑是否顺畅？概念是否解释清楚？是否为初学者写？是否与 outline 中列出的知识点一一对应？
...
询问时需要确认两件事：
1. 是否需要配图
2. 如果需要，使用什么方式生成（例如：使用codex的 imagen skills，或者使用某个图像生成的API等）
...
- 为每节课生成一张配图，要求：
  ...
  - 配图风格统一，与课程整体调性一致
- **存放位置**：必须保存到对应 `lesson{i}/` 文件夹下，命名为 `cover.png`
- **必须在该课的 `outline.md` 顶部嵌入该封面图**，作为本节的封面，例如：
  ```markdown
  ![第 N 节封面](cover.png)
  ```
  使用相对路径（与 `outline.md` 同目录），放在 H1 标题之后、正文之前

**2. 整个课程的整体封面图**
- 为整个课程生成一张总封面图，要求：
  - **文字部分**：体现课程总体主题和定位
  - **图像部分**：体现课程整体调性，与各节课封面风格统一
- **存放位置**：保存到课程根目录，命名为 `cover.png`
- **必须在根目录 `README.md` 顶部嵌入该封面图**，例如：
  ```markdown
  ![课程封面](cover.png)
  ```
  放在 H1 标题之后、课程简介之前
...
讲义的定位是**为项目做铺垫**，不是百科全书。每节课产出两份文档，承担不同角色：

### 3.1 概要 `outline.md`

定位：让学生在 1-2 分钟内**快速浏览**本节将学习的内容，建立认知地图。

- **简要列点**：以列表形式列出本节核心知识点，每条 1-2 句说明即可
- **不做展开**：不深入原理、不写示例代码、不解释 API 细节
- **结构清晰**：可包含"本节学习目标"、"涉及知识点"、"对应项目任务"几个简短小节
- **长度参考**：通常半页到一页之内

### 3.2 详细讲义 `handout.md`

定位：学生进入项目前的**主要学习材料**，对 outline 中列出的知识点逐一展开。

- **充分展开**：每个核心概念配解释、示例（必要时含示意代码）、易错点说明
- **精炼克制**：每节课仍聚焦 2-3 个核心概念，讲清楚就停，不堆砌无关内容
- **给出路标**：底层原理和进阶内容附上补充链接，不必在讲义内全部展开
- **服务项目**：每个概念的讲解都应指向"你马上要在项目中用到这个"
- **与 outline 对齐**：handout 中讲解的知识点必须与 outline 中列出的一致，不遗漏、不偏题
...
| 配图生成后未嵌入文档 | 每节封面必须嵌入对应 `lesson{i}/outline.md`；课程整体封面必须嵌入根目录 `README.md` |
